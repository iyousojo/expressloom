...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ensure $logistics_db is properly initialized
    $logistics_db = new mysqli('localhost', 'root', '', 'cms_db');
    if ($logistics_db->connect_error) {
        die("Connection to logistics database failed: " . $logistics_db->connect_error);
    }
    ...
    if (!$payment_type_check) {
        die("Error preparing payment type validation query: " . $logistics_db->error);
    }
    ...
    $stmt = $conn->prepare("INSERT INTO parcels (user_id, reference_number, sender_name, sender_contact, sender_address, recipient_name, recipient_address, recipient_contact, type, from_branch_id, to_branch_id, weight, height, width, length, price, payment_method_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"); // Updated column name
    if (!$stmt) {
        die("Error preparing parcel insertion query: " . $conn->error);
    }
