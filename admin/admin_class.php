...
Class Action {
	...
	public function __construct() {
		...
		$this->db = $logistics_db; // Use logistics_db instead of conn
	}
	function __destruct() {
	    $this->db->close();
	    ob_end_flush();
	...
	function login() {
		...
		$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM users WHERE email = '$email' AND password = '$hashed_password'");
		if ($qry->num_rows > 0) {
			foreach ($qry->fetch_array() as $key => $value) {
				if ($key != 'password' && !is_numeric($key))
					$_SESSION['login_' . $key] = $value;
			}
			$_SESSION['login_firstname'] = $_SESSION['login_firstname'] ?? ''; // Ensure this key is set
			return 1; // Login successful
		} else {
			error_log("Login failed for email: $email"); // Log failed login attempts
