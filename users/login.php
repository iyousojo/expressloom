...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->num_rows > 0 && password_verify($password, $hashed_password)) {
        $_SESSION['login_id'] = $id; // Set the session variable for user ID

        // Fetch the user's full name and account type from the database
        $user_result = $conn->query("SELECT first_name, last_name, account_type FROM users WHERE id = $id");
        if ($user_result && $user_result->num_rows > 0) {
            $user_data = $user_result->fetch_assoc();
            $_SESSION['login_firstname'] = $user_data['first_name']; // Set the first name in the session
            $_SESSION['login_lastname'] = $user_data['last_name'];   // Set the last name in the session
            $_SESSION['account_type'] = $user_data['account_type']; // Set the account type in the session
        }
